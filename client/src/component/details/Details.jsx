...
export default function Details() {
    ...
    const { gameId } = useParams(); // gameId е името на Id-то на пътя зададен в App.jsx
    ...
    useEffect(() => {
        ...
        .then (result => setGame(result)) // може и .then(setGame)
        .catch(err => alert(err.message))
    }, ([gameId])) // при промяна на id-то ще се преизпълни и ще зареди новия path с новото id

    const deleteGameHandler = async () => {
        const isComfirmed = confirm(`Are you sure you want to detele game: ${game.title}`);
        ...
        try {
            ...
            alert('Unable to delete the game ', error.message)
        }
    }
    return (
